...
function fromB64url(s: string) {
  ...
  const bin = atob(s); return new Uint8Array([...bin].map(c=>c.charCodeAt(0)));
}
...
export async function isAuthed(env: Env, request: Request) {
  ...
  try { const [payloadB64, sig] = token.split('.'); const payload = new TextDecoder().decode(fromB64url(payloadB64));
    const { exp } = JSON.parse(payload); if (!exp || Date.now() > exp) return false;
    const expect = await signHMAC(env.ADMIN_KEY, payload); return expect === sig;
  } catch { return false; }
}
