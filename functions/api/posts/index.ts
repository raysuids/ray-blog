...
export const onRequestGet: PagesFunction<Env> = async ({ env, request }) => {
  ...
  const { results } = await env.DB.prepare('SELECT id, title, content, created_at FROM posts ORDER BY created_at DESC LIMIT ? OFFSET ?').bind(pageSize, offset).all();
...
export const onRequestPost: PagesFunction<Env> = async ({ env, request }) => {
  ...
  await env.DB.prepare('INSERT INTO posts (id, title, content, created_at) VALUES (?, ?, ?, ?)')
    .bind(id, title, content, created_at).run();
  return json({ id, title, created_at });
};
