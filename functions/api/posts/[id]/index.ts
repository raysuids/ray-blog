...
export const onRequestGet: PagesFunction<Env> = async ({ env, params }) => {
  const id = (params as any).id as string; if (!id) return text('缺少 id', 400);
  const row = await env.DB.prepare('SELECT id, title, content, created_at, cover_url FROM posts WHERE id = ?').bind(id).first();
  if (!row) return text('不存在', 404); return json(row);
};
export const onRequestPut: PagesFunction<Env> = async ({ env, request, params }) => {
  const id = (params as any).id as string; if (!(await isAuthed(env, request))) return text('未授權', 401);
  let body: any; try { body = await request.json(); } catch { return text('JSON 錯誤', 400); }
  const title = (body.title || '').toString().trim(); const content = (body.content || '').toString().trim();
  const cover_url = (body.cover_url || '').toString().trim(); if (!title || !content) return text('缺少欄位', 400);
...
export const onRequestDelete: PagesFunction<Env> = async ({ env, request, params }) => {
  ...
  await env.DB.prepare('DELETE FROM comments WHERE post_id = ?').bind(id).run(); return text('OK');
};
